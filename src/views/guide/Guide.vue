<template>
	...
	<div class="guide-wrap">
		...
		<ul class="guide-trigger">
			<li class="guide-trigger-item" v-for="( guide_item , guide_index) in guide_arr" @click="guideTrigger(guide_index)" :class="{ 'guide-active-trigger': guide_active_index == (guide_index + 1) }">
			</li>
		</ul>
		<!--/引导页索引-->
		<!--跳过按钮-->
		<svg class="guide-jump" @click="jumpGuide()" v-if=" (guide_arr.length ) == guide_active_index">
				...
				_this.$http.get('http://ostbiefv1.bkt.clouddn.com/guideData.json').then(function(res) {
					...
					for(var i = 0; i < res.data.guidePage.length; i++) {
						...
						if(res.data.guidePage[i]) {
							_this.guide_arr.push(res.data.guidePage[i]);
						}
					}
				}).catch(function(err) {
					Indicator.close();
