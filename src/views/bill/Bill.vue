<template>
	<div class="container-view">
		<!--主体内容-->
		<div class="container-box" :class="{'open-menu': is_open_menu}">
			<x-header :left-options="{showBack: false}">账单
				<a slot="right">
			...
			<div class="filter-menu">
				<x-header style="background-color: #58B7FF" :left-options="{showBack: false}">筛选</x-header>
				<ul class="input-warp">
					<li class="input-item">
						<datetime title="年：" v-model="year_value" format="YYYY" confirm-text="完成" cancel-text="取消">
						</datetime>
					</li>
					<li class="input-item" v-show="year_value">
						<datetime title="月：" v-model="month_value" format="MM" confirm-text="完成" cancel-text="取消">
						</datetime>
					</li>
					<li class="input-item" v-show="month_value">
						<datetime title="日：" v-model="day_value" format="DD" confirm-text="完成" cancel-text="取消">
						</datetime>
					</li>
				</ul>
				<div class="menu-type-wrap">
					<checker v-model="check_value_arr" type="checkbox" default-item-class="bill-type-check-item" selected-item-class="bill-type-check-item-selected">
			...
			<div class="bill-wrap">
                ...
                -->
				<scroller v-show="bill_arr.length" lock-x height="-118" :scrollbarY="true" @on-scroll="onScroll" ref="billScrollEvent">
					<ul class="bill-list">
						<li class="bill-item" v-for="(bill_item,bill_index) in bill_arr">
							<span class="bill-item-type" :class="{'earn-type': bill_item.consumption_or_earn == 1,
...
<script>
	import { Scroller, Datetime, Checker, CheckerItem, XDialog, XHeader, Actionsheet, TransferDom, ButtonTab, ButtonTabItem } from 'vux'
