<template>
	<div class="consumption-wrap">
		<x-header title="消费"></x-header>
		<ul class="input-warp">
			<li class="input-item input-required">
				<calendar v-model="date_value" title="日期：" disable-future></calendar>
			</li>
			<li class="input-item input-required">
				<datetime title="时间：" v-model="time_value" format="HH:mm" confirm-text="完成" cancel-text="取消">
				</datetime>
			</li>
			<li class="input-item input-required">
				<popup-picker title="消费类型：" :data="account_type_arr" v-model="account_type">
				</popup-picker>
			</li>
			<li class="input-item input-required">
...
<script>
	import { PopupPicker, Datetime, Calendar, XInput, XHeader, Actionsheet, TransferDom, ButtonTab, ButtonTabItem } from 'vux'
	...
	export default {
		...
		data() {
			return {
				...
				remarks_value: '',
				account_type_arr: [
					['水果零食', '餐饮伙食', '出行旅游', '网上购物', '生活日常', '租房水电', '医疗药物', '其它消费']
				],
				account_type: ['水果零食'],
				date_value: 'TODAY',
